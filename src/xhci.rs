...
impl PciXhciDriver {
    ...
    async fn init_port(xhc: &Rc<Controller>, port: usize) -> Result<u8> {
        ...
        info!("xchi: port {port} is enabled");
        let slot = xhc
...
impl OperationalRegisters {
    ...
    const CMD_HC_RESRT: u32 = 0b0010;
    fn usbsts(&self) -> u32 {
        self.usbsts.read()
    }
    fn page_size(&self) -> Result<usize> {
        let page_size_bits = self.pagesize.read() & 0xFFFF;
        // bit[n] of page_size_bits is set => PAGE_SIZE wo;; be 2^(n+12).
    ...
    fn reset_xhc(&mut self) {
        ...
        self.set_command_bits(Self::CMD_HC_RESRT);
        while self.usbcmd.read() & Self::CMD_HC_RESRT != 0{
            busy_loop_hint();
        }
    }
...
impl EndpointContext {
    ...
    fn new_control_endpoint(
        ...
        // 6.2.3: Spftware shall set Average TRB Length to '8'
...
impl DeviceContext {
    ...
    fn set_last_valid_dci(&mut self, dci: usize) -> Result<()> {
        ...
        // structure.
        // - 6.2.2.2:
        // A 'validInput Slot Context for a Configure Endpoint Command
...
impl GenericTrbEntry {
    ...
    fn cmd_result_ok(&self) -> Result<()> {
        if self.trb_type() != TrbType::CommandCompletionEvent as u32 {
            Err("Not a CommandCpmpletionEvent")
...
impl CommandRing {
    ...
    fn push(&mut self, mut src: GenericTrbEntry) -> Result<u64> {
        ...
        if ring.current().trb_type() == TrbType::Link as u32 {
            // Reached to Link TRB. Let's skip i and toggle the cycle.
            ring.advance_index(!self.cycle_state_ours)?;
            self.cycle_state_ours = !self.cycle_state_ours;
        }
...
impl EventWaitInfo {
    fn matched(&self, trb: &GenericTrbEntry) -> bool {
